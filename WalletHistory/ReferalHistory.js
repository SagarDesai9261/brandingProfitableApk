...
const ReferralHistory = ({ route }) => {
    ...
    const fetchData = async () => {
        try {
            isButtonLoad(true)
            const response = await axios.get(`https://api.brandingprofitable.com/referral_history/history/${mobileNumber}`);
                        ...
                        <View key={index} style={styles.transactionContainer}>
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', }}>
                                ...
                                <View style={{ flexDirection: 'row', gap: 15, justifyContent: 'center' }}>
                                    ...
                                    <View>
                                        <Text style={[styles.transactionText1, { fontSize: 15 }]}>{item.fullName}</Text>
                                        <Text style={styles.transactionText2}>Name - {item.mobileNumber}</Text>
                                        {item?.message && <Text style={[styles.transactionText2,{color:'lightgrey'}]}>{item.message}</Text>}
                                ...
                                <View style={{ alignItems: 'flex-end', position:'absolute', right:0 }}>
                                    <Text style={[styles.transactionText1, { fontSize: 20 }]}>₹ {item.refer_amount}</Text>
                            ...
                            <View>
                                <Text style={[styles.transactionText2, { alignSelf: 'flex-end', fontSize: 13, opacity: 0.7 }]}>{item.createdAt.split(' ')[0]} - {item.createdAt.split(' ')[1]}</Text>
                            </View>
