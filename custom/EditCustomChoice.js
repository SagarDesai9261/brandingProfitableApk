...
const EditHome = ({ route, navigation }) => {
  ...
  const renderItem = useCallback(({ item }) => (
    <TouchableOpacity onPress={() => handleImagePress(item)}>
      <FastImage source={{ uri: "https://cdn.brandingprofitable.com/" +item.imageUrl }} style={styles.image} />
    </TouchableOpacity>
  ), []);
  ...
  return (
    <LinearGradient colors={['#050505', '#1A2A3D']} locations={[0, 0.4]} style={styles.container}>
      ...
      <View style={styles.imageContainer}>
        <Image source={{ uri: "https://cdn.brandingprofitable.com/" +item.imageUrl }} style={styles.mainImage} />
        <TouchableOpacity
