...
const EditHome = ({ route, navigation }) => {
  ...
  const fetchDataForImages = async () => {
    try {
      const apiUrl = `https://api.brandingprofitable.com/ds_item/dynamicdata/${bannername}`;
      ...
      const images = data.data.filter((item) => item.isVideo === false);
      ...
      if (videos.length === 0) {
        ...
        setSelectedVideo('https://cdn.brandingprofitable.com/' + videos[0].ds_itemImage);
      }
      ...
      if (images.length !== 0) {
        setItem("https://cdn.brandingprofitable.com/" + images[0].ds_itemImage);
  ...
  //   if (i < 2) {
  //     if (items.length > 0) {
  //       const ds_itemImage = items[0].ds_itemImage == undefined ? "https://static.vecteezy.com/system/resources/previews/023/914/428/non_2x/no-document-or-data-found-ui-illustration-design-free-vector.jpg" : "https://cdn.brandingprofitable.com/" + items[0].ds_itemImage
  //       setItem(ds_itemImage)
  //       seti(i)
  //     }
  //   } else {
  ...
  const handleImagePressV = (item) => {
    ...
    setSelectedVideo("https://cdn.brandingprofitable.com/" + item.ds_itemImage);
  };
  ...
  const captureAndShareImageServerSide = async (img) => {
    try {
      ...
      if (userTeamDetails === 'Purchase' && !subscritionExp) {
        ...
        if (!userPurchasedPlan.category_image) {
          Alert.alert(
  ...
  const captureAndShareVideoWithOverlay = async (imageSource, selectedVideo) => {
    ...
    if (userTeamDetails === 'Purchase' && !subscritionExp) {
      ...
      if (!userPurchasedPlan.category_video) {
        Alert.alert(
          'Please Upgrade a Plan..!',
          'Upgrade your plan to share images/videos',
          [
  ...
  const renderItem = useCallback(({ item }) => (
    <TouchableOpacity onPress={() => handleImagePress("https://cdn.brandingprofitable.com/" + item.ds_itemImage)}>
      <FastImage source={{ uri: "https://cdn.brandingprofitable.com/" + (item.comp_iamge || item.ds_itemImage) }} style={styles.image} onLoadEnd={() => Image.prefetch("https://cdn.brandingprofitable.com/" + (item.comp_iamge || item.ds_itemImage))} />
    </TouchableOpacity>
  ), []);
  ...
  //       <FastImage source={{ uri: "https://cdn.brandingprofitable.com/" + item.ds_itemImage }} style={styles.image} onLoadEnd={() => Image.prefetch("https://cdn.brandingprofitable.com/" + item.ds_itemImage)} />
  ...
  //   return (
  //     <TouchableOpacity onPress={() => handleImagePressV(item)}>
  //       <FastImage source={{ uri: "https://cdn.brandingprofitable.com/"+item.ds_itemImage, priority: FastImage.priority.high, cache: FastImage.cacheControl.cacheOnly}} resizeMode={FastImage.resizeMode.contain} style={styles.image} />
  //       <View style={{ position: 'absolute', top: 45, left: 45, zIndex: 1 }}>
  //         <Icon name="play-circle" size={30} color={"white"} />
  //       </View>
  //     </TouchableOpacity>
  ...
  //     </View>
  //     <Video
  //       source={{ uri: "https://cdn.brandingprofitable.com/" + item.ds_itemImage }}
  //       repeat={false}
  //       paused={false}
  //       style={styles.image}
  //       resizeMode="cover"
            ...
            (
              ...
              /> */}
                ...
                <PagerView
                  ...
                  style={{ height: width - 50, width: width - 50, marginVertical: 20, elevation: 20, }}
                  initialPage={0}
                >
