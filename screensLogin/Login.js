...
import { View, StyleSheet, TextInput, Image, Text, Alert, ActivityIndicator, Dimensions, TouchableHighlight, TouchableOpacity, ScrollView, ToastAndroid, BackHandler } from 'react-native';
...
const LoginScreen = () => {
  ...
  const fetchUserRole = async () => {
    try {
      ...
      console.log("userRole in login screeen", userRole)
      if (userRole) {
        await AsyncStorage.removeItem("UserVipTag")
      }

      const userRoleAfterRemove = await AsyncStorage.getItem('UserVipTag');
      console.log("userRoleAfterRemove in login screeen", userRoleAfterRemove)
    } catch (error) {
  ...
  const handleLogin = async () => {
    ...
    if (phone.length !== 10) {
      ...
      try {
        ...
        console.error("login screen error send otp", error);
      } finally {
        setIsLoader(false);
      }
      setPhone('')
  ...
  const handleSuccessGoogleLogin = async (details) => {
    ...
    const apiUrl = `https://api.brandingprofitable.com/user/email/${profileData.email}`;
  ...
  //       }).catch((e) => {
  //         console.log("ERROR IS IN GOOGLE LOGIN: " + JSON.stringify(e));
  //       })
  //     }
  //   }).catch((e) => {
  ...
  const signIn = async () => {
    GoogleSignin.configure({
      webClientId: "403941213111-st88ncj8ipetcasa9nd1g54jebaj7bkf.apps.googleusercontent.com",
    });
    GoogleSignin.hasPlayServices().then((hasPlayService) => {
      if (hasPlayService) {
        GoogleSignin.signOut();
        ...
        GoogleSignin.signIn().then((userInfo) => {
          ...
          console.log("ERROR IS in the func: " + JSON.stringify(e));
        })
      }
    }).catch((e) => {
      console.log("ERROR IS: " + JSON.stringify(e));
